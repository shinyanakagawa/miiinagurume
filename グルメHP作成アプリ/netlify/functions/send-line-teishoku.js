...
exports.handler = async (event) => {
  ...
  if (resendKey && notifyEmail) {
    const html = `
      <div style="font-family:'Helvetica Neue',Arial,sans-serif;background:#fdf8f2;padding:32px;color:#2e1f10;">
        <div style="max-width:480px;margin:0 auto;background:#ffffff;border:1px solid #e6d8c8;border-radius:8px;overflow:hidden;">
          <div style="background:#c0613a;padding:24px 32px;">
            <p style="margin:0;color:rgba(255,255,255,.85);font-size:11px;letter-spacing:.3em;text-transform:uppercase;">New Reservation</p>
            <h1 style="margin:8px 0 0;color:#ffffff;font-size:22px;letter-spacing:.08em;">ごはん処 みいな</h1>
          </div>
          <div style="padding:32px;">
            <h2 style="margin:0 0 20px;font-size:16px;color:#c0613a;">ご予約を受け付けました</h2>
            <table style="width:100%;font-size:14px;border-collapse:collapse;">
              <tr><td style="padding:10px 0;border-bottom:1px solid #e6d8c8;color:#9e7d63;width:96px;">お名前</td><td style="padding:10px 0;border-bottom:1px solid #e6d8c8;font-weight:bold;">${escapeHtml(name)}</td></tr>
              <tr><td style="padding:10px 0;border-bottom:1px solid #e6d8c8;color:#9e7d63;">電話番号</td><td style="padding:10px 0;border-bottom:1px solid #e6d8c8;">${escapeHtml(tel)}</td></tr>
              <tr><td style="padding:10px 0;border-bottom:1px solid #e6d8c8;color:#9e7d63;">日付</td><td style="padding:10px 0;border-bottom:1px solid #e6d8c8;">${escapeHtml(date)}</td></tr>
              <tr><td style="padding:10px 0;border-bottom:1px solid #e6d8c8;color:#9e7d63;">時間</td><td style="padding:10px 0;border-bottom:1px solid #e6d8c8;">${escapeHtml(time)}</td></tr>
              <tr><td style="padding:10px 0;color:#9e7d63;">人数</td><td style="padding:10px 0;">${escapeHtml(pax)}名</td></tr>
            </table>
          </div>
          <div style="background:#f5ede0;padding:16px 32px;text-align:center;font-size:11px;color:#9e7d63;letter-spacing:.05em;">
            ごはん処 みいな ご予約管理
          </div>
        </div>
      </div>
    ...
    try {
      const emailRes = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          from: 'Gohandokoro MIINA <onboarding@resend.dev>',
          to: [notifyEmail],
